...
public class SearchServiceImpl implements SearchService {
	...
	public List<ShowSummaryResponse> searchShows(String query) {
		...
		List<Long> showIds = searchResult.stream().map(result -> result.show().id()).toList();

		Map<Long, List<CommentSummary>> commentsByShowId = commentRepository.findCommentGroupsByShowIdIn(showIds).stream()
				.collect(Collectors.toMap(CommentGroup::id, CommentGroup::comments));

		return searchResult.stream().map(result -> mapToShowSummaryResponse(result,
				commentsByShowId.getOrDefault(result.show().id(), List.of()))).toList();

	}

	private ShowSummaryResponse mapToShowSummaryResponse(TvMazeSearchResult searchResult, List<CommentSummary> comments) {
		return new ShowSummaryResponse(searchResult.show().id(), searchResult.show().name(),
				resolveChannelName(searchResult.show()), searchResult.show().summary(), searchResult.show().genres(),
				comments);
	}
